...
def train(hyp, opt, device, tb_writer=None):
    # hyperparameters：超参数
    # opt：训练参数
    # 超参数：保存路径、训练轮数、batchsize、权重，rank
    logger.info(colorstr('hyperparameters: ') + ', '.join(f'{k}={v}' for k, v in hyp.items()))
    save_dir, epochs, batch_size, total_batch_size, weights, rank = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank
    # Directories创建并保存权重目录
    ...
    # Configure
    # opt参数没有plots参数 创建plots
    plots = not opt.evolve  # create plots
    cuda = device.type != 'cpu'
    # 设置随机种子，在神经网络中，参数默认是进行随机初始化的。如果不设置的话每次训练时的初始化都是随机，
    ...
    # Weights & Biases
    # 自动记录模型训练过程中的超参数和输出指标，然后可视化和比较结果
    ...
    # 获取类别数目,如果设置opt.single_cls,则为一类
    nc = 1 if opt.single_cls else int(data_dict['nc'])  # number of classes
    # 获取类别姓名,如果设置opt.single_cls,则为item
...
'''
关于预训练模型：
        可通过opt.cfg，也可通过ckpt['model'].yaml
        ...
        设置了intersect_dicts，该函数就是忽略掉exclude中的键对应的值
'''
    if pretrained:
        # 加载预训练模型,则如果本地不存在则从Google上下载
        with torch_distributed_zero_first(rank):
            weights = attempt_download(weights)  
        # ckpt:checkpoint 加载检查点
